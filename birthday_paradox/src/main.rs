...
(It's not actually a paradox, it's just a surprising result.)"
    ...
    _ = io::stdin()
        ...
        .expect("Failed to read line.");
    println!("Let's run another {} simulations.", SIMU_NUMS);
...
fn paradox_simulation(num_birthdays: u32) -> Vec<NaiveDate> {
    let mut random_birthdays = Vec::new();
    let mut rng = rand::rng();

    while random_birthdays.len() < num_birthdays.try_into().unwrap() {
        if let Some(day) = NaiveDate::from_yo_opt(2001, rng.random_range(1..=365)) {
